...
public class DataBoardController {
	...
	public String databoard_list(String page, Model model) {
		// Model model : JSP / HTML => 데이터 전송 객페 = request 대체
		if(page == null)
			page = "1";
		int curpage = Integer.parseInt(page);
		int rowSize = 10;
		int start = (curpage-1)*rowSize; // OFFSET => 0부터 시작
		                                 // rownum => 1부터 시작
	...
	public void databoard_download(String fn, HttpServletResponse response) {
		try {
			...
			// 2. header전송 : 파일명 / 파일크기 => 다운로드 창을 보여준다
	...
	@ResponseBody
	// 화면 이동 => HTML / JavaScript에 필요한 데이터 전송
	// 단점 : 한글이 깨진다
