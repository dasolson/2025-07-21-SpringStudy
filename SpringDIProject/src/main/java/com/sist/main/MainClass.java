 ...
 *                  | => AnnotationConfigApplicationContext(자바 사용)
 *    *** WebApplicationContext : core => AOP(공통모듈) + MVC 
 *    
 *        => 주입 => 생성 / 소멸 관리 한다
 *                   | new를 사용하지 않고 컨테이너를 통해서 객체을 얻어서 사용
 *                   | new 사용은 ~VO(데이터형)
 *        => 방식
 *            1) XML
 *            2) Annotation
 ...
 *             -----------------
 *             *** 모든 객체는 싱글턴을 이용한다 (저장된 객체 주고사 변경이 안된다)
 *             *** getBean()을 호출 할때 마다 => 객체 생성이 가능
 *                 scope="singleton" scope="prototype"
 *          3) DI 수행 => setter, 생성자를 이용한 데이터 주입
 *          4) 요청 메소드 호출
 *              = 객체 생성시 => init-method
