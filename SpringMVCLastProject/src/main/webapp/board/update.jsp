...
<%--
		Vue
			...
			View      : mount에 소속된 HTML
			             <template>
			ViewModel : 요청 처리 => Model에 있는 데이터 변경
			                               | 값 변경시마다
			                               | View 전송
			1. Vue 객체 생성
				Vue.createApp({})
			2. Vue에서 HTML을 제어하는 데이터를 정의
			  ...
			  ----------------------------------- Model
			   ...
			   mounted(){}  => HTML이 브라우저에서 실행이 된 상태
			       |             => window.onload = function()
			                     ...
			                     ** 시작과 동시에 서버에 데이터 읽기
			                     ** 다른 프레인워크 연동
			                     ** Jquery / React / AngulaJS 
			   updated(){}  => data() 안에 잇는 데이터 값이 갱신되면 호출
			       |
			  unmounted(){} => 화면 변경이 된 경우 =>Vue 객체 소멸
			  ...
			  컴포넌트 기반
			  ----- UI 재사용 할 수 있게 만든 독립적인 블록
			         => data, props, methods
			         => 기능별로 나눠서 저장 후 필요한 경우 재사용
			         => pagecard                                                                        
